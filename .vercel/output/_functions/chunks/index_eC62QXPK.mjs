...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const group = categoryGroups.find((g) => g.slug === "uitjes");
  const allValues = group.subcategories.map((s) => s.sanityValue);
  const [pillar, posts] = await Promise.all([
    sanityClient.fetch(`*[_type == "pillarPage" && slug.current == "uitjes"][0]{ title, label, intro, heroImageUrl, contentHtml, metaDescription }`),
  ...
  const title = pillar?.title ?? "Uitjes met kids";
  const label = pillar?.label ?? "Vakantie voor Kids";
  const intro = pillar?.intro ?? "Op zoek naar een leuk uitje met de kinderen? Van binnenspeeltuinen tot musea en zwemparadijzen.";
  const metaDescription = pillar?.metaDescription ?? intro;
  const subcategories = group.subcategories.map((sub) => ({
    slug: sub.slug,
    title: sub.title,
    description: sub.description,
    href: `/uitjes/${sub.slug}`
  }));
...
}, "/Users/daanjangrotenhuis/Sanity/src/pages/uitjes/index.astro", void 0);

const $$file = "/Users/daanjangrotenhuis/Sanity/src/pages/uitjes/index.astro";
const $$url = "/uitjes";
